...
from src.utils import jsonl_gz_to_parquet, build_corpus_from_parquet, save_corpus_parquet, load_parquet
...
if not reviews_parquet.exists():
    jsonl_gz_to_parquet(
        root / f"data/raw/{CATEGORY}.jsonl.gz",
        reviews_parquet,
...
if not meta_parquet.exists():
    jsonl_gz_to_parquet(
        root / f"data/raw/meta_{CATEGORY}.jsonl.gz",
        meta_parquet
    )
else:
